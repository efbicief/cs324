...
function init() {
  ...
  scene.background = new THREE.Color( 0x393933 );
  ...
  const floorMaterial1 = new THREE.MeshPhongMaterial( { color: 0xccccdd, flatShading: true, shininess: 10 } );
  ...
  loader2.load('../assets/models/Nature/FBX/CommonTree_Snow_1.fbx', function(fbx) {
...
// Check if a rail spline is valid - i.e. if there is a too sharp bend
...
function drawLineFromPoints(points, colour) {
  try {
    ...
    console.log(points); // YOU WERE HERE
  }
...
function drawRails(railSplineInfo, drawSpline) {
  ...
  if (railSplineInfo.points != undefined && railSplineInfo.points.length > 1) {
    ...
    if (invalidCurve) {
      document.getElementById("info").innerHTML = "Bend too sharp!";
    }
...
function moveTrain(train) {
  if (train.pointIndex < train.journey.points.length - 1) {
    ...
    train.lamp.position.copy(train.journey.points[train.pointIndex]);
    try {
      train.model.lookAt(train.journey.points[train.pointIndex+1]);
      train.model.position.y += 13;
      train.lamp.position.y += 13;
...
function render() {
  ...
  for (let i=0; i<railSplines.length; i++) { // OVERHAUL
      ...
      if (generatedTownNames !== undefined && timeElapsed.toFixed(1) % 1 === 0) {
        for (const townName1 of generatedTownNames) {
          for (const townName2 of generatedTownNames) {
            stationDemands[townName1].waiting[townName2] += (Math.random() < stationDemands[townName1].demand[townName2] ? 2 : 0);
          }
        }
      }
    }
  ...
  if (unpaused) {
    ...
    for (let i=0; i<journeys.length; i++) {
      if (journeys[i].lastTrain != timeInSecs && timeInSecs % journeys[i].frequency === 0) {
        ...
        let newLamp = new THREE.PointLight( 0xfcf9d9, 5, 100 );
        newLamp.castShadow = true;
        scene.add(newLamp);
        trains.push({ model: trainScene.clone(), journey: journeys[i], pointIndex: 0, visitedStations: [], onBoard: peopleOnBoard, lamp: newLamp })
